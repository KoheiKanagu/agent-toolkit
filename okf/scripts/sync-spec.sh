...
SPEC_PATH="$REPO_ROOT/okf/references/SPEC.md"
...
if cmp -s "$TMP_FILE" "$SPEC_PATH"; then
  echo "okf/references/SPEC.md is already up to date."
  exit 0
fi

if [[ "$MODE" == "check" ]]; then
  echo "The vendored OKF spec is out of date."
  echo "Please update okf/references/SPEC.md to match the upstream spec."
  exit 1
fi

cp "$TMP_FILE" "$SPEC_PATH"
echo "Updated okf/references/SPEC.md from upstream."
